            ...
            <table id="example" class="table table-vcenter table-condensed table-bordered">
                ...
                <tbody id="tableData">
                    ...
                    <tr id="master{{$n->id}}">
                        ...
                        <td>
                            <div class="btn-group">
                                <a href="{{route('admin.master.edit', $n->slug)}}" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default edit"><i class="fa fa-pencil"></i></a>
                                @if(Auth::guard('admin')->user()->role == 1)
                                  <a href="javascript:void(0)" data-toggle="tooltip" title="Delete" class="btn btn-xs btn-danger remove_button" url="{{route('admin.master.destroy', $n->id)}}"><i class="fa fa-times"></i></a>
                                @endif
                            </div>
                        </td>
                    </tr>
...
{{-- for change status switchery --}}
   <script>
        ...
        $('tbody').delegate('.status-switch', 'change', function(){
            ...
            $.post('{{route('admin.change.master.status')}}',{'_token':'{{csrf_token()}}',news_id:newsId, status:chk}, function(){
