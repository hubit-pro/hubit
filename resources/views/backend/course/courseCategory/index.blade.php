...
<div id="page-content">
    <div class="block full">
        <div class="block-title">
            <h2><strong>Course</strong> Category</h2>
        ...
        <div class="table-responsive">
            <table id="example" class="table table-vcenter table-condensed table-bordered">
                ...
                <tbody id="tableData">
                    ...
                    <tr id="courseCategory{{$category->id}}">
                        <td>{{++$key}}</td>
                        ...
                        <td>
                            <div class="btn-group">
                                <a href="javascript:void(0)" url="{{route('admin.course_category.edit', $category->id)}}" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default edit" data-category="{{$category}}"><i class="fa fa-pencil"></i></a>
                                <a href="javascript:void(0)" data-toggle="tooltip" title="Delete" class="btn btn-xs btn-danger remove_button" url="{{route('admin.course_category.destroy', $category->id)}}"><i class="fa fa-times"></i></a>
                            </div>
                        </td>
                    </tr>
                    @empty
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('admin.course_category.store')}}" method="post" enctype="multipart/form-data" class="form-horizontal form-bordered" onsubmit="return false;"  id="addForm">
                    @csrf
                    <fieldset>
...
{{-- for change status switchery --}}
   <script>
        ...
        $('tbody').delegate('.status-switch', 'change', function(){
            ...
            $.post('{{route('admin.change.course.category.status')}}',{'_token':'{{csrf_token()}}',category_id:categoryId, status:chk}, function(){
...
{{-- for add  --}}
    ...
    <script type="text/javascript">
        $('#example').delegate('.edit', 'click', function(){
            ...
            $('#editForm').attr('action','/course_category/'+category.id);
